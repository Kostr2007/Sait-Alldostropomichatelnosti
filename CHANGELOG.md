...
Каждый **30 дней** в **15 числа**, будут собирать из обновление **глобальное обновление**. Со скриншотами, выводами и так далее.

## [0.42.53] - [2026-07-08]

### ✨ Добавлено

- Переработан профиль. (Добавить в будущем подержку сохранение аккаунта при выходе и входе.)

### 📝 Комментарии

<details>
<summary> 📝 Дневник разработчика </summary>
Привет-привет! Знаешь? Когда лень что то делать так что даже тут приходится делить задание по немногу! Проект не умер он просто перешёл на стадию как по мне улитки! Чтобы мне было легче работать и после не возникло желание бросить на потом. Так что... удачи будущему мне ^~^!
</details>
...
- Профиль теперь интегрирован в supabase.
- Изменён название в `lib/supabase.ts` внутри файла на `PUBLIC_SUPABASE_ANON_KEY`.

### 📝 Комментарии

<details>
<summary> 📝 Дневник разработчика </summary>
Привет! 5 дней ничего не делал? Как бы не так... я делал только не спеша... дела в том что пока что... я в деревне и мне как то тяжело войти в ритм... проект тяжёлый и засидеться полноцено не сломав себе нервые клетки я не знаю! А мне как бы хочется отдыхать и все дела так что... проект будет делаться! Но сами коммиты могу не отправить. Так что... спасибо за понимания... будущий я 😅💤.
</details>
...
Наконец, производственая и практика закончена... и теперь могу снова вкатиться в проект.
Так как мне проект придётся занова вкатываться... тоесть привыкать... то возрощаться буду постепенно.
И на сегодня я всего лишь напишу этот текст как начало.

## [0.40.49] - [2026-06-08] по [2026-06-19] (Когда закончится производственная практика) - Временная заморзка проекта

![Статус](https://img.shields.io/badge/Статус-Micro%20Updates-f63d45?style=for-the-badge)

### 📝 Комментарии разработчика

Привет! Так как у меня уже начинается скажем так... производственная практика... где мне надо выполнять сразу два задание... по производственной и по предмету... я бы не хотел нагружать свой же день! Я и так и rastrover делаю и проект последние время занимает не привычные 10-15 минут... а целые даже **1 час!!!** Так как проект усложняется и приходится как ни странно... **ДУМАТЬ**! Так или иначе проект **не забрасывается**... и чтобы время не терять я тут создам таблицу **идеи** чтобы во время своих выполнение других задание... я мог что то добавить в капилку!

[Изменено] Мне предложили идею дробить задачи... так что... проект всё таки **БУДЕТ РАБОТАТЬ**, но! Скажем так обновления будут на уровне простых! Добавить что то маленькое и подредактировать... Так что добавляю следующий раздел **микро-обновления**. 😺

### 🎉 Идеи

- Форумы и рецензий известных людей.
  - Зачем это? Так как я подумал... что сайт должен и удержать человека не только информаций про достопримечательностей... но и предоставить пространство для других чтобы люди могли обсуждать разные темы.
- Дзэн мод.
  - Зачем это? Приятно звуко сопровождения и анимация создаёт уют... а учитывая с форумом это может создать приятную атмосферу для общения!

### 💤 Микро-обновления

- В `profile.astro`
  - Добавлено 12 колонок после `hr`
  - были добавлены `grid` и `gap` тоесть заготовка для столбцов.
  - Был добавлен `div` с занимающие 5 колонок под информацию об аккаунте (слева).
  - Немного по практиковался в `tailwindcss`.
  - Теперь и правая колонка распределеная по сетке.
  - Добавлен `ul`

## [0.39.48] - 2026-06-07 - Добавление профиля

![Статус](https://img.shields.io/badge/Статус-Unreleased-2ec27e?style=for-the-badge)

### ✨ Добавлено

- Файл `profile.astro` как заглушка с гостём.

### ♻️ Изменено

- Попутно было изменено в `Header` маршрут на профиль. (Из за того что маршруты это под категория карт).
- Был доконца написан `[slug].astro`.

### 📝 Комментарии

Сайт был уже протестирован на моём личном сервере VNC через `nginx`. Сайт на телефоне работает приемлемо (проблемы с лагами в анимациях и при карте).

<details>
<summary> 📝 Дневник разработчика </summary>
Что тебе скажу? Намечается на очень много работы... сейчас надо профиль обьединить и создать бд! А также создание достопримечательностей удобную сделать в сайте. И привезать и к пользователю. И репорты и прочие прочие делать! Короче... много чего придётся сделать... и защиту некто не отменял. Ну так как время у меня безлимитное и значит париться мне не зачем... лишь будет муторно. 🛌😫💤
</details>

## [0.38.47] - 2026-06-6 - Переработка

![Статус](https://img.shields.io/badge/Статус-Unreleased-2ec27e?style=for-the-badge)

### ✨ Добавлено

- Добавлен `src/DOCUMENTATION-DESIGN.md` где будут последовательно рассказывать как делается дизайн сайта.

### ♻️ Изменено

- Весь `README.md`, разделив техническую часть и эмоциональную.

### 📝 Комментарии

Было произведена большая работа над лицовой части проекта и последующие заготовки.

<details>
<summary> 📝 Дневник разработчика </summary>
Ну... что сказать то мне? **ПИЗДЕЦ!** Так как я просидел тут гораздо больше чем на написание кода... мнда! Но теперь проект выглядит реально как проект... Эмоций не куда не денутся лишь будут отделяться от технической. Так что... проект живой! Только теперь он стал более техничным... Хотя может он потерял свой шарм... хотя врятли я всё равно буду использовать эмодзи! 😼
</details>
